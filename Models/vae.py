...
class VAEDeepStable(nn.Module):
    ...
    def __init__(self, latent_dim, input_shape):
        ...
        - input_shape (tuple): Shape of the input data (e.g., (1, num_channels, height, width)).
        """
        super(VAEDeepStable, self).__init__()
        ...
        self.flatten = nn.Flatten()
        self.fc_enc1 = nn.Linear(fc_size, 128)
        self.bn_enc1 = nn.BatchNorm1d(128)
        self.fc_enc2 = nn.Linear(128, 64)
        self.bn_enc2 = nn.BatchNorm1d(64)
        self.fc_enc3_mean = nn.Linear(64, latent_dim)
        self.fc_enc3_logvar = nn.Linear(64, latent_dim)

        # Decoder layers
        self.fc_dec1 = nn.Linear(latent_dim, 64)
        self.bn_dec1 = nn.BatchNorm1d(64)
        self.fc_dec2 = nn.Linear(64, 128)
        self.bn_dec2 = nn.BatchNorm1d(128)
        self.fc_dec3 = nn.Linear(128, fc_size)
        self.bn_dec3 = nn.BatchNorm1d(fc_size)
        self.unflatten = nn.Unflatten(1, (64, height // 8, width // 8))
        self.deconv1 = nn.ConvTranspose2d(64, 32, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.bn_deconv1 = nn.BatchNorm2d(32)
        self.deconv2 = nn.ConvTranspose2d(32, 16, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.bn_deconv2 = nn.BatchNorm2d(16)
        self.deconv3 = nn.ConvTranspose2d(16, num_channels, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.bn_deconv3 = nn.BatchNorm2d(num_channels)
    ...
    def encode(self, x):
        ...
        Returns:
        - tuple: Tuple containing mean and logvar of the latent space.
        """
        x = F.leaky_relu(self.bn1(self.conv1(x)), negative_slope=self.neg_factor)
        x = self.pool(x)
        x = F.leaky_relu(self.bn2(self.conv2(x)), negative_slope=self.neg_factor)
        x = self.pool(x)
        x = F.leaky_relu(self.bn3(self.conv3(x)), negative_slope=self.neg_factor)
        x = self.pool(x)
        x = self.flatten(x)
        x = F.leaky_relu(self.bn_enc1(self.fc_enc1(x)), negative_slope=self.neg_factor)
        x = F.leaky_relu(self.bn_enc2(self.fc_enc2(x)), negative_slope=self.neg_factor)
    ...
    def decode(self, z):
        ...
        """
        z = F.leaky_relu(self.bn_dec1(self.fc_dec1(z)), negative_slope=self.neg_factor)
        z = F.leaky_relu(self.bn_dec2(self.fc_dec2(z)), negative_slope=self.neg_factor)
        z = F.leaky_relu(self.bn_dec3(self.fc_dec3(z)), negative_slope=self.neg_factor)
        z = self.unflatten(z)
        z = F.leaky_relu(self.bn_deconv1(self.deconv1(z)), negative_slope=self.neg_factor)
        z = F.leaky_relu(self.bn_deconv2(self.deconv2(z)), negative_slope=self.neg_factor)
        x_hat = torch.sigmoid(self.bn_deconv3(self.deconv3(z)))
    ...
    def forward(self, x):
        ...
        Returns:
        - tuple: Tuple containing reconstructed data, mean, and logvar of the latent space.
        """
        z_mean, z_logvar = self.encode(x)
        z = self.reparameterize(z_mean, z_logvar)
        x_hat = self.decode(z)
