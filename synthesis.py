...
def load_data(file_path):
    ...
    x, y, metadata, CLASSES = data['x'], data['y'], data['metadata'].tolist(), data['classes']
...
def visualize_results(x_spec_batch, x_hat, y_batch, CLASSES, sr, hop_length, n_samples=10, alpha=1.0, color='b', filename="Original_vs_Reconstructed"):
    ...
    with tqdm(total=len(x_spec_batch), desc="Visualizing Results") as pbar:
...
if __name__ == "__main__":
    ...
    x_hat, _, _ = process_batch(x_spec_batch, vae, device, pbar)
    visualize_results(x_spec_batch, x_hat, y_batch, CLASSES, sr, hop_length, n_samples=5, alpha=1.0, color='b', filename="Original_vs_Reconstructed111")

    # Sample Generation
    num_samples = 5  # Number of sounds to generate
    ...
    animate_generated_samples(generated_mel_spectrograms, sr, hop_length, n_samples=num_samples, alpha=1.0, color='b', output_path="./Figures/Generated_Samples111.gif")
